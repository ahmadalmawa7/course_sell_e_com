  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (admin === 'true') {
        ...
        return res.status(200).json(allTickets.map(ticket => ({ ...ticket, id: ticket._id.toString() })));
      }
      ...
      return res.status(200).json(userTickets.map(ticket => ({ ...ticket, id: ticket._id.toString() })));
    }
    ...
    if (req.method === 'POST') {
      ...
      return res.status(201).json({ ...ticket, id: result.insertedId.toString() });
    }
    ...
    if (req.method === 'PATCH') {
      ...
      const result = await tickets.findOneAndUpdate({ _id: new ObjectId(id) }, { $set: updatePayload }, { returnDocument: 'after' });
      if (!result.value) {
